...
def calc_bops(model, input_data_precision=32):
    ...
    for module in model.layers:
        if isinstance(module, Conv2D) or isinstance(module, QConv2D) or isinstance(module, Dense) or isinstance(module, QDense): 
            ...
            b_w = int(module.get_quantization_config()['kernel_quantizer'].split('(')[-1].split(',')[0]) + 1  # plus 1 to account for sign bit qkeras ignores
            print("BITWIDTH: ", b_w)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Calculate model BOPS')
    parser.add_argument('-m', '--model', required=True)
    args = parser.parse_args()

    calc_bops(load_model(args.model))